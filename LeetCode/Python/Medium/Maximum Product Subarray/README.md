...
### 📜 Problem Description
<p>Given an integer array <code>nums</code>, find a <span data-keyword="subarray-nonempty" class=" cursor-pointer relative text-dark-blue-s text-sm"><button type="button" aria-haspopup="dialog" aria-expanded="false" aria-controls="radix-_r_ff_" data-state="closed" class="">subarray</button></span> that has the largest product, and return <em>the product</em>.</p>
